...
>(({ step, children, className, onStepperItemClick, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div
        className={cn('flex items-center gap-2', isVerticalLabel && !isVertical && 'flex-col')}
        ref={(node) => {
          if (scrollTracking) {
            node?.scrollIntoView({
              behavior: 'smooth',
      ...
      >
        <Button
          ...
          onClick={onClickItem}
          className={cn(
            'stepper-item-button aspect-square h-6 w-6 rounded-full p-1',
        ...
        <div className={cn('w-max text-start', isVerticalLabel && !isVertical && 'text-center')}>
          {steps[step].label && (
            <p className="text-xs text-slate-900 dark:text-slate-100">{steps[step].label}</p>
          )}
